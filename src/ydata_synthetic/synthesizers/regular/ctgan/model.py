...
class CTGAN(BaseGANModel):
    ...
    def sample(self, n_samples: int):
        ...
        data_df = self.processor.inverse_transform(data)
        new_ids = np.arange(1, n_samples + 1)
        data_df['ID'] = new_ids

        # Correct negative values in numeric columns
        num_cols_to_correct = ['Amount', 'Funded Amount', 'Funded Amount Investor', 'Term', 'Interest Rate',
        ...
        for col in num_cols_to_correct:
